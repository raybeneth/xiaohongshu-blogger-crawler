...
async def reasons_for_purchase_attr(
        pi, page, filter_labels, parent_count, filter_count, child_count,
        parent_name
) -> None:
    # 以「属性买点」标题 div 为锚点，跳到父级作为整个模块的作用域容器，
    # 子卡片列表和第一组筛选项均收窄在此容器内，避免误匹配页面其他区域
    attr_module = page.locator(
        '.title:has(span.tooltip-title:has-text("属性买点"))'
    ).locator('xpath=..').first

    # 第一组 segment-control 容器，限定在「属性买点」模块内
    first_segment = attr_module.locator('[style*="segment-control-padding"]').first
    ...
    for fi, filter_label in enumerate(filter_labels):
        if fi == 0:
            ...
            # 在「属性买点」模块的第一组 segment-control 内切换筛选项
        ...
        for ci, child_name in enumerate(CHILD_CARD_NAMES):
            ...
            # 子卡片限定在「属性买点」模块内定位
            child_card = attr_module.locator(
...
async def note_type_card_crawler(
        pi: int, parent_count: int, parent_name: str, filter_labels: tuple[str, str], page: Page
) -> None:
    """
    抓取「笔记类型」卡片数据。
    该卡片只受父卡片影响，每个父卡片点击后抓取一次。
    筛选项与父卡片联动，直接传入 PARENT_FILTER_MAP[parent_name] 的值。
    """
    filter_count = len(filter_labels)
    print(
        f"  [父 {pi + 1}/{parent_count}][笔记类型]"
        f" 开始抓取，共 {filter_count} 个筛选项：{list(filter_labels)}"
    )
    logger.info(
        "  [父 %d/%d][笔记类型] 开始抓取，筛选项：%s",
